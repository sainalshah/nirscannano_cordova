...
import com.sitf.scanplugin.NanoPlugin;
...
public class NanoBLEService extends Service {
    ...
    private ArrayList<byte[]> scanConfList = new ArrayList<byte[]>();
    private ArrayList<byte[]> storedScanList = new ArrayList<byte[]>();
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            ...
            if (descriptor.getCharacteristic().getUuid().compareTo(KSTNanoSDK.NanoGATT.GCIS_RET_REF_CAL_COEFF) == 0) {
                ...
                LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(notifyCompleteIntent);
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (characteristic.getUuid().equals(KSTNanoSDK.NanoGATT.DIS_MANUF_NAME)) {
                    ...
                    LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(intent);
                    ...
                    LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(scanConfSizeIntent);
                    if (debug)
                    ...
                    LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(sdScanSizeIntent);
                    ...
                    if (!activeConfRequested) {
                        ...
                        LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(sendActiveConfIntent);
                    } else {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            if (characteristic == KSTNanoSDK.NanoGattCharacteristic.mBleGattCharGSDISStartScanNotify) {
                ...
                LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(scanStartedIntent);
                scanData.reset();
                refConf.reset();
                refMatrix.reset();
                size = 0;
                ...
                if (data[0] == 0x00) {
                    ...
                    LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(requestCalCoef);
                } else {
                    int i;
                    for (i = 1; i < data.length; i++) {
                ...
                if (data[0] == 0x00) {
                    ...
                    LocalBroadcastManager.getInstance(NanoPlugin.mContext).sendBroadcast(requestCalMatrix);
                } else {
                    int i;
                    for (i = 1; i < data.length; i++) {
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (characteristic.getUuid().equals(KSTNanoSDK.NanoGATT.GSDIS_START_SCAN)) {
                ...
                String dataString = SettingsManager.getStringPref(NanoPlugin.mContext, SettingsManager.SharedPreferencesKeys.prefix, "Data");
                ...
                if (!scanStarted) {
                    ...
                    byte[] data = {0x00};
                    if (SettingsManager.getBooleanPref(NanoPlugin.mContext, SettingsManager.SharedPreferencesKeys.saveSD, false)) {
                        data[0] = 0x01;
                        if (debug)
    ...
    public void onCreate() {
        ...
        mStartScanReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                Log.d(NanoPlugin.TAG,"mStartScanReceiver, Received start scan broadcast!!");
                scanStarted = true;
                String dataString = SettingsManager.getStringPref(NanoPlugin.mContext, SettingsManager.SharedPreferencesKeys.prefix, "Nano");
                if (dataString.equals("")) {
                    dataString = "Nano";
                }
                byte[] data = new StringBuilder(dataString).reverse().toString().getBytes();
        ...
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mDataReceiver, new IntentFilter(KSTNanoSDK.SEND_DATA));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mInfoRequestReceiver, new IntentFilter(KSTNanoSDK.GET_INFO));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mStatusRequestReceiver, new IntentFilter(KSTNanoSDK.GET_STATUS));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mScanConfRequestReceiver, new IntentFilter(KSTNanoSDK.GET_SCAN_CONF));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mStoredScanRequestReceiver, new IntentFilter(KSTNanoSDK.GET_STORED_SCANS));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mSetTimeReceiver, new IntentFilter(KSTNanoSDK.SET_TIME));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mStartScanReceiver, new IntentFilter(KSTNanoSDK.START_SCAN));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mDeleteScanReceiver, new IntentFilter(KSTNanoSDK.DELETE_SCAN));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mGetActiveScanConfReceiver, new IntentFilter(KSTNanoSDK.GET_ACTIVE_CONF));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mSetActiveScanConfReceiver, new IntentFilter(KSTNanoSDK.SET_ACTIVE_CONF));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mUpdateThresholdReceiver, new IntentFilter(KSTNanoSDK.UPDATE_THRESHOLD));
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).registerReceiver(mRequestActiveConfReceiver, new IntentFilter(KSTNanoSDK.REQUEST_ACTIVE_CONF));
    }
    ...
    public void onDestroy() {
        ...
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mDataReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mInfoRequestReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mStatusRequestReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mScanConfRequestReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mStoredScanRequestReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mSetTimeReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mStartScanReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mDeleteScanReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mGetActiveScanConfReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mSetActiveScanConfReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mUpdateThresholdReceiver);
        LocalBroadcastManager.getInstance(NanoPlugin.mContext).unregisterReceiver(mRequestActiveConfReceiver);
    }
